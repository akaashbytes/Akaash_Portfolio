      ...
      <AnimatePresence>
        ...
        {status === 'error' && errorMsg && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: -20, scale: 0.9 }}
            className="fixed top-24 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-6 py-4 glassmorphism-premium border-red-500/30 rounded-2xl shadow-2xl shadow-red-500/10"
          >
            <AlertCircle className="text-red-500" size={24} />
            <div className="text-left">
              <h5 className="text-sm font-bold text-white uppercase tracking-wider">Delivery Failed</h5>
              <p className="text-xs text-zinc-300">{errorMsg}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
